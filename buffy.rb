...
set :github, Octokit::Client.new(:access_token => ENV['GITHUB_PAT_OCTOKIT'])
# set :joss_api_key, ENV['JOSS_API_KEY'] # need to change
set :ropensci_editor_team_id, 2419827 # need to change
set :magic_word, "bananas"
set :editors, ['maelle', 'noamross', 'karthik', 'sckott']

# Before we handle the request we extract the issue body to grab the ropenscibot
# command (if present).
before do
...
def robawt_respond
  ...
  case @message
  when /\A@ropenscibot commands/i
    respond erb :commands
  when /\A@ropenscibot assign (.*) as reviewer/i
    check_editor
    assign_reviewer($1)
    respond "OK, the reviewer is #{$1}"
  when /\A@ropenscibot assign (.*) as editor/i
    check_editor
    assign_editor($1)
    respond "OK, the editor is #{$1}"
  when /\A@ropenscibot set (.*) as archive/
    check_editor
    assign_archive($1)
  when /\A@ropenscibot start review magic-word=(.*)|\A@ropenscibot start review/i
    check_editor
  ...
  when /\A@ropenscibot list editors/i
    respond erb :editors, :locals => { :editors => editors }
  when /\A@ropenscibot list reviewers/i
    respond reviewers
  end
end
...
## FIXME: we don't have a public reviewers list as of yet
## - get from Airtable
def reviewers
  "Here's the current list of rOpenSci reviewers: <coming soon...>"
end

# Return an array of editor usernames for rOpenSci editor list
def editors
  @editors ||= settings.github.team_members(settings.ropensci_editor_team_id).collect { |e| e.login }
end
...
def update_assigness(assignees)
  ...
  url = "https://api.github.com/repos/#{@nwo}/issues/#{@issue_id}/assignees?access_token=#{ENV['GITHUB_PAT_OCTOKIT']}"
  RestClient.post(url, data.to_json)
end

## FIXME: will need to change this
# def start_review
#   editor = issue.body.match(/\*\*Editor:\*\*\s*.@(\S*)/)[1]
...
def check_editor
  unless settings.editors.include?(@sender)
    respond "I'm sorry @#{@sender}, I'm afraid I can't do that. That's something only rOpenSci Onboarding editors are allowed to do."
    halt 403
  end
end
